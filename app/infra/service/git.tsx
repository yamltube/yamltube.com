...
import type {
  ...
  WhoAmIResult,
  Workspace,
} from "@pulumi/pulumi/automation/workspace";
import type { OutputMap } from "@pulumi/pulumi/automation/stack";
import { Stack } from "@pulumi/pulumi/automation/stack";
import type { ProjectSettings } from "@pulumi/pulumi/automation/projectSettings";
import type { StackSettings } from "@pulumi/pulumi/automation/stackSettings";
import { minimumVersion } from "@pulumi/pulumi/automation/minimumVersion";
import type { ConfigMap, ConfigValue } from "@pulumi/pulumi/automation/config";
import { CommandResult, runPulumiCmd } from "./cmd";

// Copy pasted from:
// https://github.dev/pulumi/pulumi/blob/350274c99637f5ad1c2f442f3b8e4ce2a23599ae/sdk/nodejs/automation/stackSettings.ts#L38
const stackSettingsSerDeKeys = [
...
/**
 * GitWorkspace is a default implementation of the Workspace interface.
...
export class GitWorkspace implements Workspace {
   ...
   */
  static async create(opts: LocalWorkspaceOptions): Promise<GitWorkspace> {
    const ws = new GitWorkspace(opts);
    await ws.ready;
    return ws;
  }
  /**
  ...
  static async createOrSelectStack(
    ...
    if (isInlineProgramArgs(args)) {
      return await this.inlineSourceStackHelper(
        args,
        Stack.createOrSelect,
        opts
      );
    } else if (isLocalProgramArgs(args)) {
      return await this.localSourceStackHelper(
        args,
        Stack.createOrSelect,
        opts
      );
  ...
  private static async localSourceStackHelper(
    ...
    const ws = new GitWorkspace(wsOpts);
    await ws.ready;
  ...
  private constructor(opts?: LocalWorkspaceOptions) {
    ...
    const readinessPromises: Promise<any>[] = [
      this.getPulumiVersion(minimumVersion),
    ];
  ...
  async stackSettings(stackName: string): Promise<StackSettings> {
    ...
    for (const ext of settingsExtensions) {
      ...
      const path = upath.joinSafe(
        this.workDir,
        `Pulumi.${stackSettingsName}${ext}`
      );
    ...
    throw new Error(
      `failed to find stack settings file in workdir: ${this.workDir}`
    );
  }
  /**
  ...
  async saveStackSettings(
    ...
    const path = upath.joinSafe(
      this.workDir,
      `Pulumi.${stackSettingsName}${foundExt}`
    );
  ...
  async upGitRepo(repo: string): Promise<void> {
    const initResult = await this.runCmd(["$", "git", "init"]);
    if (initResult.code === 0) {
      console.log("initialized git repo");
    }
    const remoteAddResult = await this.runCmd([
      "$",
      "git",
      "remote",
      "add",
    ...
    if (remoteAddResult.code === 0) {
      console.log(`added remote repo ${repo}`);
    }
  }

  async pushRepo(remoteName: string): Promise<void> {
    const pushResult = await this.runCmd([
      "$",
      "git",
      "push",
      remoteName,
  ...
  async selectStack(stackName: string): Promise<void> {
    await this.runCmd(["stack", "select", stackName]);
  }
  /**
  ...
  async removeStack(stackName: string): Promise<void> {
    await this.runCmd(["stack", "rm", "--yes", stackName]);
  }
  /**
  ...
  async removeConfig(stackName: string, key: string): Promise<void> {
    await this.runCmd(["config", "rm", key, "--stack", stackName]);
  }
  /**
   *
  ...
  async removeAllConfig(stackName: string, keys: string[]): Promise<void> {
    await this.runCmd(["config", "rm-all", "--stack", stackName, ...keys]);
  }
  /**
  ...
  async refreshConfig(stackName: string): Promise<ConfigMap> {
    await this.runCmd(["config", "refresh", "--force", "--stack", stackName]);
  ...
  async whoAmI(): Promise<WhoAmIResult> {
    const result = await this.runCmd(["whoami"]);
  ...
  async listStacks(): Promise<StackSummary[]> {
    const result = await this.runCmd(["stack", "ls", "--json"]);
    return JSON.parse(result.stdout);
  }
  /**
  ...
  async installPlugin(
    ...
    await this.runCmd(["plugin", "install", kind, name, version]);
  }
  /**
  ...
  async listPlugins(): Promise<PluginInfo[]> {
    const result = await this.runCmd(["plugin", "ls", "--json"]);
  ...
  async stackOutputs(stackName: string): Promise<OutputMap> {
    ...
    const maskedResult = await this.runCmd([
      "stack",
      "output",
      "--json",
      "--stack",
    ...
    const plaintextResult = await this.runCmd([
      "stack",
      "output",
      "--json",
      "--show-secrets",
  ...
  private async getPulumiVersion(minVersion: semver.SemVer) {
    const result = await this.runCmd(["version"]);
    const optOut =
      !!this.envVars[SKIP_VERSION_CHECK_VAR] ||
      !!process.env[SKIP_VERSION_CHECK_VAR];
    const version = parseAndValidatePulumiVersion(
      minVersion,
      result.stdout.trim(),
      optOut
    );
  ...
  private async runCmd(args: string[]): Promise<CommandResult> {
 ...
 */
function isLocalProgramArgs(
  args: LocalProgramArgs | InlineProgramArgs
): args is LocalProgramArgs {
 ...
 * @param args The args object to evaluate
 */
function isInlineProgramArgs(
  args: LocalProgramArgs | InlineProgramArgs
): args is InlineProgramArgs {
  return (
    (args as InlineProgramArgs).projectName !== undefined &&
