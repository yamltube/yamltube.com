...
import { InlineProgramArgs, LocalWorkspace } from "@pulumi/pulumi/automation/localWorkspace";
...
export class Service {
    ...
    async up<T>(name: string, program: () => Promise<T>, providers: string[] ): Promise<T> {
            // Create our stack 
            ...
            for (let provider of providers) {
                await stack.workspace.installPlugin(provider.split("/")[0], provider.split("/")[1])
            }
            await stack.refresh()
    ...
    async shamatches(triplet: string, head: string): Promise<boolean> {
        const resp = await fetch(`https://api.pulumi.com/api/stacks/${triplet}/updates?pageSize=1`, {
